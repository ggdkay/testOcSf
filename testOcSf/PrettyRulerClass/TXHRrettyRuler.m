...
- (void)setRulerMinValue:(NSInteger)minCount{
    rulerScrollView.rulerMinCount = minCount ;

}

- (void)setRulerMaxValue:(NSInteger)maxCount{
    rulerScrollView.rulerMaxCount = maxCount;
    
}

//-132.0 == >start at 87.0
 CGFloat PER_DISTANCE = 80;
- (void)setRulerValue:(CGFloat)rulerValue{
   CGFloat disValue = rulerValue - rulerScrollView.rulerValue  ;
 
   CGFloat currentOffset = rulerScrollView.contentOffset.x ;
    rulerScrollView.contentOffset = CGPointMake(PER_DISTANCE*disValue+currentOffset, 0.f);
  //  NSLog(@"rulerScrollView %f ,cur %f",currentOffset,rulerScrollView.rulerValue);
  
    rulerScrollView.mode = YES ;
    [self scrollViewDidScroll:rulerScrollView];
    
}

- (void)showRulerScrollViewWithCount:(NSInteger)maxCount
    ...
    NSAssert(currentValue < [average floatValue] * maxCount, @"***** currentValue 不能大于直尺最大值（count * average）\n");
...
- (void)scrollViewDidScroll:(TXHRulerScrollView *)scrollView {
  //    NSLog(@"scrollView.mode set  %f", scrollView.contentOffset.x);
    ...
    if (ruleValue < 0.f) {
        return;
    } else if (ruleValue > (scrollView.rulerMaxCount-scrollView.rulerMinCount) * [scrollView.rulerAverage floatValue]) {
        return;
    }
